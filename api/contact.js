...
function badHeader(s) { return /[\r\n]/.test(s || ""); }
function parseCookie(req) {
  return (req.headers.cookie || "").split(";").reduce((acc, p) => {
    const [k, ...v] = p.trim().split("="); if (!k) return acc;
...
function verifyToken(token) {
  try {
    ...
    const expected = crypto.createHmac("sha256", process.env.CONTACT_SECRET).update(`${ts}.${nonce}`).digest("hex");
